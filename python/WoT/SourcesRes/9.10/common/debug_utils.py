...
def CRITICAL_ERROR(msg, *kargs):
    print _makeMsgHeader('CRITICAL ERROR', sys._getframe(1)),
    print msg,
    print kargs
    if IS_CLIENT:
        import BigWorld
...
def LOG_WRONG_CLIENT(entity, *kargs):
    if hasattr(entity, 'id'):
        entity = entity.id
    print _makeMsgHeader('WRONG_CLIENT', sys._getframe(1)),
    print entity,
    print kargs
...
def trace(func):
    ...
    def wrapper(*args, **kwds):
        print '[%s] (%s, %d) call %s:' % ('DEBUG',
         ...
         fname),
        print ':',
        print ', '.join(('%s=%r' % entry for entry in zip(argnames, args) + kwds.items()))
        ret = func(*args, **kwds)
...
+++ okay decompyling debug_utils.pyc 
